...
async def update_user_role(
    ...
    if not await require_admin(current_user_id, session):
        return ApiResponse(code=403, message="无权限"), 403

    result = await session.execute(select(User).where(User.id == user_id))
    user = result.scalar_one_or_none()
    if not user:
        return ApiResponse(code=404, message="用户不存在"), 404

    new_role = body.get("role", "creator")
    try:
        user.role = UserRole(new_role)
    except ValueError:
        return ApiResponse(code=400, message="无效角色"), 400

    await session.commit()
    return ApiResponse(message="角色已更新")


@router.put("/admin/users/{target_id}/active")
